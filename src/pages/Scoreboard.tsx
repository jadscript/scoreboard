...
const Divider = () => <div className="w-[2px] h-[45px] bg-gray-300 rounded-full" />;
  ...
  return (
    <div className="flex flex-col items-center justify-between">
      <div className="w-full grid grid-cols-2 items-center justify-center h-screen">
        <div className="flex absolute top-0 left-0 px-6 py-2 m-3 bg-white text-black rounded-full items-center gap-4 shadow-md">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              <div className={`w-2.5 h-2.5 rounded-full bg-black transition-opacity duration-200 ${serving === 'team1' ? 'opacity-100' : 'opacity-0'}`} />
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-blue-600 rounded-full" />
              <span className="font-bold text-sm mr-2">Team 2</span>
              <div className={`w-2.5 h-2.5 rounded-full bg-black transition-opacity duration-200 ${serving === 'team2' ? 'opacity-100' : 'opacity-0'}`} />
            </div>
          </div>
          ...
          {Array.from({ length: 3 }, (_, i) => {
            ...
            return (
              <div key={i} className="flex flex-col gap-1 items-center">
                <span className={`font-bold text-sm w-3 text-center ${team1Won === false ? 'opacity-40' : ''}`}>{t1}</span>
                <span className={`font-bold text-sm w-3 text-center ${team1Won === true ? 'opacity-40' : ''}`}>{t2}</span>
              </div>
            );
          })}
          ...
          <button
            onClick={handleReset}
            className="text-sm font-semibold text-red-500 transition-colors cursor-pointer hover:bg-red-500 hover:text-white rounded-full px-3 py-3"
          >
            Reset
          </button>
        </div>
